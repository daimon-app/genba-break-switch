...
## 公開中PWAの実画面確認（2026-08-19 JST）

GitHub Pagesの公開URL `https://daimon-app.github.io/genba-break-switch/` を実際に確認した。タイトルは「現場休憩スイッチ」で、ホーム画面には通常／暑い日／重作業の3モード、時間休憩、30秒／1分／3分／5分のマイクロリセット、端末ローカルの統計、記録、瞑想・整えタイマーが表示された。初期画面は縦長の1画面内に収まり、主操作ボタンは押下対象として視認できる。

この確認は**表示時点の初期画面**に限る。終了音、バックグラウンド時の動作、各モード遷移、アクセシビリティ、端末実機差、インストール後の動作は、QA Stageで別途検証する。

| 確認項目 | 結果 | 根拠 |
| --- | --- | --- |
| 公開URL | 到達可 | GitHub Pages実画面 |
| HTML title | 「現場休憩スイッチ」 | 実ブラウザ表示 |
| ホームの主要導線 | 表示可 | 通常／暑い日／重作業、時間休憩、各マイクロリセット、記録、瞑想・整えタイマー |
| モバイル優先の密度 | 1画面内に主要導線を配置 | 実画面確認。対象ビューポートでスクロール不要 |
| 販売名称との整合 | 未整合 | 受領仕様書の候補名「切り替えスイッチ」に対し、公開画面・HTML・manifestは旧称 |

> **TECH_FIX_REQUIRED候補:** 正式商品名が本人承認された後、公開画面、HTML title、manifest、Apple Web App title、GitHub Pagesメタデータ、各販売文書で名称を一括整合する。
